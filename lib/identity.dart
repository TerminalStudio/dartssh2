...
class Identity {
  ...
  Ed25519Key getEd25519PublicKey() =>
      Ed25519Key(Uint8List.fromList(ed25519!.verifyKey));

  Ed25519Signature signWithEd25519Key(Uint8List m) =>
      Ed25519Signature(ed25519!.sign(m).buffer.asUint8List(0, 64));
  ...
  List<MapEntry<Uint8List, String>> getRawPublicKeyList() {
    final ret = <MapEntry<Uint8List, String>>[];
    if (ed25519 != null) {
...
bool verifyEd25519Signature(
  ...
  return VerifyKey(publicKey.key!).verify(
...
bool verifyRSASignature(
  ...
  return signer.verifySignature(
    message,
    asymmetric.RSASignature(signature.sig!),
  );
}
